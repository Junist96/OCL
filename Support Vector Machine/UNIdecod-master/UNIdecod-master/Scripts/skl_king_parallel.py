...
# update    2014 01 15: add SVR and LogisticRegression options
...
def default(var, value, dict): #if the value (var) is provided, uses it. Otherwise, uses the by-default value (value).
    if var in dict:
        out = dict[var]
    else:
        out = value
...
clf_type = default("clf_type", "SVC",mat);
C = default("C", 1, mat)
...
save_coef = default("save_coef", False, mat) #init=True
...
def my_pipeline(train, test,
    ...
    if fb_fold:
        sys.stdout.write("<")
        sys.stdout.flush()
    # initialize results within a given folds
    (predict, predictg, probas, probasg, distance, distanceg) = ([], [], [], [], [], [])
    if compute_predict:
    ...
    for d in range(0, dims.shape[0]):
        ...
        if np.unique(ytrain).shape[0] > 1:
            ...
            if np.size(C)>1:
                ...
                clf = _clf
            clf.fit(Xtrain, ytrain, sample_weight=sw_train) # won't work with logistic & LinearSVC.
            if save_coef:
